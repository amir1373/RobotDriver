...
static uint16_t WriteSingleHoldingRegister(const uint8_t *pucQuery, uint8_t *pucResponse)
{
    ...
    if ((m_tModbusData.psHoldingRegisterHigherLimit[usStartAddress] >= (uint16_t) usRegisterValue) &&
        (m_tModbusData.psHoldingRegisterLowerLimit[usStartAddress] <= (uint16_t) usRegisterValue))
    {
...
static uint16_t WriteMultipleHoldingRegisters(const uint8_t *pucQuery, uint8_t *pucResponse)
{
    ...
    while (usTmpNumOfData > 0)
    {
        ...
        if ( !((m_tModbusData.psHoldingRegisterHigherLimit[usStartAddress] >= (uint16_t) usValue) &&
            (m_tModbusData.psHoldingRegisterLowerLimit[usStartAddress] <= (uint16_t) usValue)))
        {
            bException = true;
        }
